...
/**
 * @brief Configures a CTRE TalonSRX with only the fields provided.  All other
 * fields are given the factory default values.
 *
...
bool TalonSRXConfig(WPI_TalonSRX& motorController,
  ...
  if constexpr (has_peakCurrentLimit<T>()) {
    ...
    static_assert(currentLimit.to<double>() > 0,
  ...
  if constexpr (has_peakCurrentDuration<T>()) {
    ...
    static_assert(currentDuration.to<double>() > 0,
  ...
  if constexpr (has_statusFrameMotorMode<T>()) {
    argos_lib::status_frame_config::SetMotorStatusFrameRates(
        motorController, T::statusFrameMotorMode);
  }
...
/**
 * @brief Configures a CTRE TalonSRX with configuration values according to
 * specified robot instance.
 *
